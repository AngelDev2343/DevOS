...
#
# ERRORES CORREGIDOS (encontrados en producción):
...
# 9.  systemctl en chroot poco confiable → symlinks directos
...
[[ $EUID -ne 0 ]] && die "Ejecuta con sudo o como root."
...
if [[ -n "$KERNEL_VER" && ! -f "$ROOTFS/boot/vmlinuz-${KERNEL_VER}" ]]; then
    ...
    [[ -f "$VMLINUZ_SRC" ]] && \
        ...
        warn "vmlinuz no encontrado"
else
    ok "vmlinuz ya en /boot/"
fi
...
# FIX #10: graphical.target como default
ln -sf /usr/lib/systemd/system/graphical.target \
    "$ROOTFS/etc/systemd/system/default.target"

# FIX #11: deshabilitar plymouth completamente
...
ok "Servicios habilitados, plymouth deshabilitado."

# ── Perfil XFCE de kiosk ──────────────────────────────────────────────────────
log "Configurando perfil XFCE..."
...
[[ -n "$KERNEL_VER" && -f "$ROOTFS/boot/vmlinuz-${KERNEL_VER}" ]] && \
    ok "  vmlinuz OK ($KERNEL_VER)" || warn "  vmlinuz NO en /boot"
...
[[ -f "$ROOTFS/etc/systemd/system/plymouth-quit-wait.service" ]] && ok "  plymouth deshabilitado OK"
