...
[[ ! -d "$ROOTFS/usr" ]] && die "Rootfs no encontrado. Ejecuta scripts 01 y 02 primero."

for cmd in mksquashfs xorriso grub2-mkimage dracut; do
    command -v "$cmd" &>/dev/null || die "Falta: $cmd — ejecuta el script 01 primero."
done
...
if [[ ! -f "$VMLINUZ" ]]; then
    ...
    [[ -f "$VMLINUZ_ALT" ]] && cp "$VMLINUZ_ALT" "$VMLINUZ" && \
        ...
        die "vmlinuz no encontrado. Reinstala kernel-core."
fi

# ── Initramfs Live (SIN plymouth) ────────────────────────────────────────────
log "Generando initramfs Live con dracut (2-5 min)..."
# FIX #4: omitir plymouth del initramfs
...
# FIX #2 y #3: sin rd.live.overlay.overlayfs ni rd.live.overlay=tmpfs
# FIX nomodeset: necesario para VirtualBox con vmwgfx
...
echo "  Controlador gráfico: VBoxSVGA (no VMSVGA)"
echo ""
